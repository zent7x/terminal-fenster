...
/// Extract a top-level unsigned integer field (decimal digits only).
pub fn json_get_u64(json: &str, key: &str) -> Option<u64> {
    json_get_i64(json, key).and_then(|n| u64::try_from(n).ok())
}

/// Extract a top-level signed integer field (optional leading `-`, decimal digits).
pub fn json_get_i64(json: &str, key: &str) -> Option<i64> {
    let needle = format!("\"{key}\"");
    let start = json.find(&needle)? + needle.len();
    let rest = json[start..].trim_start();
    let rest = rest.strip_prefix(':')?.trim_start();
    let (neg, rest) = if let Some(r) = rest.strip_prefix('-') {
        (true, r)
    } else {
        (false, rest)
    };
...
mod tests {
    ...
    fn frame_header_roundtrip() {
        ...
        assert_eq!(
            (h.dirty_x, h.dirty_y, h.dirty_w, h.dirty_h),
            (10, 20, 30, 40)
        );
    ...
    fn a_4k_frame_is_still_under_the_cap() {
        ...
        const { assert!(3840 * 2160 * 4 + FRAME_HEADER_LEN < MAX_MESSAGE_LEN) };
    }
    ...
    #[test]
    fn json_get_i64_reads_signed_values() {
        assert_eq!(json_get_i64(r#"{"code":-105}"#, "code"), Some(-105));
        assert_eq!(json_get_u64(r#"{"matches":5}"#, "matches"), Some(5));
        assert_eq!(json_get_u64(r#"{"code":-1}"#, "code"), None);
    }
