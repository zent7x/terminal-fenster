...
pub struct Capabilities {
    ...
    /// Kitty's POSIX shared-memory transmission medium (`t=s`). This is enabled only after
    /// a real 1x1 object is opened successfully by the terminal, and never across SSH or a
    /// multiplexer where the terminal cannot necessarily see this process's namespace.
    ...
    /// DEC private mode 2026 (synchronized output). When supported, multi-tile Kitty
    /// frames can be wrapped so the terminal presents them atomically instead of
    /// painting tile-by-tile. Detected via DECRQM; absence means progressive fall-back.
...
impl Capabilities {
    ...
    pub fn viewport_px(&self) -> Option<(u32, u32)> {
        ...
        // A busy terminal can time out on the window-pixel queries (CSI 14 t and the ioctl
        // above) while still having answered CSI 16 t with a real per-cell size. Derive the
        // viewport from that cell before resorting to a guess: collapsing a detected 17x37
        // cell to an assumed 8x16 renders the page at roughly a quarter resolution, and the
        // graphics layer then upscales that small bitmap into the window as a blur.
        if ws.cols > 0 && ws.rows > 0 {
            if let Some((cw, ch)) = self.cell {
                if cw > 0 && ch > 0 {
                    return Some((ws.cols as u32 * cw as u32, ws.rows as u32 * ch as u32));
                }
            }
    ...
    /// macOS Terminal.app — no graphics protocols; interactive browsing is unsupported.
    pub fn is_apple_terminal(&self) -> bool {
        matches!(self.term_program.as_deref(), Some("Apple_Terminal"))
    }

    /// True when the terminal answered a pixel graphics capability probe.
    pub fn has_pixel_graphics(&self) -> bool {
        self.kitty_graphics || self.sixel || self.iterm2_images
    }
}
...
pub fn detect(fd: RawFd, deadline_ms: u64) -> Capabilities {
    ...
    let reply = query(
        fd,
        &kitty::support_query(kitty::GRAPHICS_PROBE_ID),
        d,
        |b| find(b, b"\x1b\\").is_some() || find(b, b"OK").is_some(),
    );
    c.kitty_graphics = find(&reply, b"OK").is_some() && find(&reply, b"_G").is_some();
    c.raw_replies
        .push(("kitty_graphics".into(), escape_for_display(&reply)));
    if c.kitty_graphics {
    ...
    // A generic Kitty graphics reply does not prove that the terminal shares our POSIX shm
    // namespace. Probe the actual medium with a real 0600 object, as required by the protocol.
    // Remote and multiplexed sessions keep the direct/zlib path until their namespace and
    // passthrough behaviour is measured end-to-end.
    if c.kitty_graphics && !c.remote && !c.in_tmux && !c.in_screen {
        match kitty::shared_memory_support_query(32) {
            Ok((seq, pending)) => {
                let reply = query(fd, &seq, d, |b| {
                    find(b, b"_Gi=32;").is_some() && find(b, b"\x1b\\").is_some()
                });
                c.kitty_shared_memory = find(&reply, b"_Gi=32;OK").is_some();
                c.raw_replies
                    .push(("kitty_shared_memory".into(), escape_for_display(&reply)));
                // If the terminal consumed it this is already unlinked; otherwise the guard
                // removes the failed/timed-out probe here.
                drop(pending);
            }
            Err(e) => c.raw_replies.push((
                "kitty_shared_memory".into(),
                format!("local probe failed: {e}"),
            )),
        }
    } else {
        c.raw_replies.push((
            "kitty_shared_memory".into(),
            if c.remote || c.in_tmux || c.in_screen {
                "skipped: non-local or multiplexed session".into()
            } else {
                "skipped: Kitty graphics unavailable".into()
            },
        ));
    }
    ...
    c.raw_replies
        .push(("kitty_keyboard".into(), escape_for_display(&reply)));
    ...
    c.raw_replies
        .push(("window_px".into(), escape_for_display(&reply)));
    ...
    c.raw_replies
        .push(("cell_px".into(), escape_for_display(&reply)));
    if c.cell.is_none() {
    ...
    c.raw_replies
        .push(("sgr_pixel_mouse".into(), escape_for_display(&reply)));

    // Synchronized output (2026) via DECRQM. Same value semantics as 1016.
    let reply = query(fd, b"\x1b[?2026$p", d, |b| ends_with_any(b, b"y"));
    c.sync_output = parse_decrqm_supported(&reply);
    c.raw_replies
        .push(("sync_output".into(), escape_for_display(&reply)));
...
fn parse_decrqm_supported(reply: &[u8]) -> bool {
    ...
    let Some(rest) = s.split("\x1b[?").nth(1) else {
        return false;
    };
...
fn parse_da1_has_sixel(reply: &[u8]) -> bool {
    ...
    let Some(params) = rest.split('c').next() else {
        return false;
    };
...
mod tests {
    ...
    #[test]
    fn apple_terminal_is_detected() {
        let c = Capabilities {
            term_program: Some("Apple_Terminal".into()),
            ..Default::default()
        };
        assert!(c.is_apple_terminal());
        let c = Capabilities {
    ...
    #[test]
    fn viewport_uses_detected_cell_when_window_pixels_are_missing() {
        // Ghostty under load: CSI 14 t (window_px) and the TIOCGWINSZ pixel fields both time
        // out, but CSI 16 t reported a real 17x37 cell. The viewport must be the full-
        // resolution cols*cell, not the 8x16 guess that would quarter the resolution and let
        // the graphics layer upscale a small bitmap into a blur.
        let c = Capabilities {
            winsize: WinSize {
                rows: 23,
                cols: 146,
        ...
        assert_eq!(c.viewport_px(), Some((2482, 851)));
    }
    ...
    fn escape_display_is_readable() {
        ...
        assert_eq!(
            escape_for_display(b"\x1b_Gi=31;OK\x1b\\"),
            "\\e_Gi=31;OK\\e\\"
        );
    }
}
