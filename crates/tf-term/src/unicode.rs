...
pub fn render_half_blocks(rgb: &[u8], w: u32, h: u32, cols: u32, rows: u32, out: &mut String) {
    ...
    for row in 0..rows {
        let mut last: Option<ColourPair> = None;
        for col in 0..cols {
...
fn dangerous_terminal_char(c: u32) -> bool {
    c < 0x20                         // C0 controls, includes ESC 0x1b
        || c == 0x7f                 // DEL
        || (0x80..=0x9f).contains(&c) // C1 controls, includes CSI 0x9b
        || c == 0x00ad               // soft hyphen
        || c == 0x061c               // Arabic letter mark
        || (0x200b..=0x200f).contains(&c) // zero-width space/joiners, LRM/RLM
        || (0x2028..=0x202e).contains(&c) // separators and bidi embedding/override
        || c == 0x2060               // word joiner
        || (0x2066..=0x2069).contains(&c) // bidi isolates
        || (0xfff9..=0xfffb).contains(&c) // interlinear annotation controls
...
pub fn sanitize_for_terminal(s: &str, max_len: usize) -> String {
    ...
    for (kept, ch) in s.chars().enumerate() {
        if kept >= max_len {
            out.push('\u{2026}');
            break;
        }
        let c = ch as u32;
        out.push(if dangerous_terminal_char(c) {
            '\u{fffd}'
        } else {
            ch
        });
...
/// Clip already-human-readable terminal chrome to a conservative display-column budget.
///
/// Exact Unicode width depends on terminal policy. Counting printable ASCII as one column and
/// every non-ASCII scalar as two intentionally overestimates ambiguous/combining characters; it
/// may shorten chrome slightly, but it cannot make page-controlled text wrap the last terminal
/// row and scroll the graphics canvas. Dangerous formatting controls are replaced again here as
/// a final-sink guard.
pub fn clip_to_terminal_columns(s: &str, max_columns: usize) -> String {
    let mut out = String::with_capacity(s.len().min(max_columns));
    let mut columns = 0usize;
    for ch in s.chars() {
        let safe = if dangerous_terminal_char(ch as u32) {
            '\u{fffd}'
        } else {
            ch
        };
        let width = if safe.is_ascii() { 1 } else { 2 };
        if columns.saturating_add(width) > max_columns {
            break;
        }
        out.push(safe);
        columns += width;
...
mod tests {
    ...
    fn half_block_encodes_top_and_bottom_separately() {
        ...
        assert!(
            ...
            "fg should be the top pixel"
        );
        assert!(
            s.contains("\x1b[48;2;0;0;255m"),
            "bg should be the bottom pixel"
        );
    }
    ...
    fn half_block_resets_sgr_each_row() {
        ...
        assert_eq!(
            ...
            2,
            "must reset per row, not leak colour"
        );
    }
    ...
    #[test]
    fn sanitize_strips_bidi_and_invisible_address_spoofing() {
        let evil = "safe.test\u{202e}moc.live\u{2066}\u{200b}\u{feff}";
        let clean = sanitize_for_terminal(evil, 100);
        for control in ['\u{202e}', '\u{2066}', '\u{200b}', '\u{feff}'] {
            assert!(!clean.contains(control));
        }
        assert!(clean.contains("safe.test"));
    }

    #[test]
    fn column_clip_is_conservative_for_wide_and_combining_text() {
        assert_eq!(clip_to_terminal_columns("abc日本語", 7), "abc日本");
        // Combining marks are deliberately over-counted. The important invariant is that the
        // returned string can never exceed the caller's terminal-column budget.
        assert_eq!(clip_to_terminal_columns("a\u{301}b", 3), "a\u{301}");
    }

    #[test]
    fn column_clip_replaces_controls_at_the_final_tty_sink() {
        let clipped = clip_to_terminal_columns("ok\u{202e}evil", 20);
        assert!(!clipped.contains('\u{202e}'));
        assert!(clipped.starts_with("ok"));
    }
}
