...
pub enum Event {
    ...
    FocusLost,
    Paste(String),
    /// Kitty graphics protocol reply (`ESC _ G … ST`). Demultiplexed out of the key
    /// stream so marker/frame ACKs cannot be typed into the page (C09 §2.6 / C01 D3).
    KittyReply {
        id: Option<u32>,
        status: String,
    },
    /// Other APC/DCS string sequences that are not user input.
...
impl Decoder {
    ...
    fn step(&mut self) -> Step {
        ...
        match self.buf[1] {
            b'[' => self.step_csi(),
            b'O' => self.step_ss3(),
            // APC / DCS carry terminal replies (Kitty graphics ACKs, tmux wrap, …).
            // Consume them as reply events — never as Alt+_ / Alt+P keystrokes.
            b'_' | b'P' => self.step_string_seq(),
            _ => {
    ...
    /// Consume an APC (`ESC _ …`) or DCS (`ESC P …`) string terminated by ST or BEL.
    fn step_string_seq(&mut self) -> Step {
        let Some((end, term_len)) = find_string_terminator(&self.buf, 2) else {
            return Step::NeedMore;
        };
        let seq: Vec<u8> = self.buf[..=end].to_vec();
        self.buf.drain(..end + 1);

        // Kitty reply: ESC _ G <keys>;<status> ST
        if seq.len() >= 3 && seq[1] == b'_' && seq[2] == b'G' {
            let payload_end = seq.len().saturating_sub(term_len);
            let payload = &seq[3..payload_end];
            if let Some((id, status)) = parse_kitty_reply(payload) {
                return Step::Event(Event::KittyReply { id, status });
            }
        }
        Step::Event(Event::TerminalReply(seq))
    }
    ...
    fn step_csi(&mut self) -> Step {
        ...
        if body.is_empty() && (final_byte == b'I' || final_byte == b'O') {
            self.buf.drain(..=end);
            return Step::Event(if final_byte == b'I' {
                Event::FocusGained
            } else {
                Event::FocusLost
            });
...
fn decode_kitty_key(body: &[u8]) -> Option<Event> {
    ...
    let (mods, kind) = if sections.len() > 1 && !sections[1].is_empty() {
        ...
        let m = parse_u32(mp[0])
            .map(Modifiers::from_kitty_param)
            .unwrap_or_default();
        let k = if mp.len() > 1 {
...
/// Locate ST (`ESC \`) or BEL terminating an APC/DCS string. Returns `(last_index, term_len)`.
fn find_string_terminator(buf: &[u8], start: usize) -> Option<(usize, usize)> {
    let mut i = start;
    while i < buf.len() {
        match buf[i] {
...
/// Parse Kitty reply payload after `ESC _ G` and before ST: `i=<id>;OK` / `i=<id>;ENOENT`.
fn parse_kitty_reply(payload: &[u8]) -> Option<(Option<u32>, String)> {
    let semi = payload.iter().position(|&b| b == b';')?;
    let meta = &payload[..semi];
    let status = std::str::from_utf8(&payload[semi + 1..]).ok()?.to_string();
    if status.is_empty() {
        return None;
    }
    let mut id = None;
    for part in meta.split(|&b| b == b',') {
        if let Some(rest) = part.strip_prefix(b"i=") {
            id = std::str::from_utf8(rest).ok()?.parse().ok();
        }
    }
    Some((id, status))
}
...
mod tests {
    ...
    fn split_escape_sequence_across_reads_is_reassembled() {
        ...
        let mut d = dec();
        assert!(
            d.decode(b"\x1b[<0;10").is_empty(),
            "partial sequence must not emit"
        );
    ...
    fn lone_escape_is_ambiguous_until_flushed() {
        let mut d = dec();
        assert!(
            d.decode(b"\x1b").is_empty(),
            "lone ESC must not resolve immediately"
        );
    ...
    #[test]
    fn kitty_graphics_reply_is_demultiplexed_not_typed() {
        // Without demux, ESC _ G … would become Escape then literal text into the page.
        let ev = dec().decode(b"\x1b_Gi=1879048193;OK\x1b\\");
        assert_eq!(
            ev,
            vec![Event::KittyReply {
                id: Some(0x7000_0001),
                status: "OK".into(),
            }]
        );
    ...
    #[test]
    fn kitty_graphics_error_reply_carries_status() {
        let ev = dec().decode(b"\x1b_Gi=2000;EBADPNG\x1b\\");
        assert_eq!(
            ev,
            vec![Event::KittyReply {
                id: Some(2000),
    ...
    #[test]
    fn kitty_reply_split_across_reads() {
        let mut d = dec();
        assert!(d.decode(b"\x1b_Gi=31;O").is_empty());
        let ev = d.decode(b"K\x1b\\");
        assert_eq!(
            ev,
            vec![Event::KittyReply {
                id: Some(31),
    ...
    #[test]
    fn kitty_reply_interleaved_with_keystrokes() {
        let ev = dec().decode(b"a\x1b_Gi=1;OK\x1b\\b");
        assert_eq!(ev.len(), 3);
        assert!(matches!(
            &ev[0],
            Event::Key {
    ...
    #[test]
    fn dcs_string_is_not_alt_p() {
        let ev = dec().decode(b"\x1bP0$r\x1b\\");
        assert!(matches!(&ev[0], Event::TerminalReply(_)));
    }
}
