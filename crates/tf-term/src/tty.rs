...
///
/// `\x1b[?2026l` MUST be first. If we die inside a DEC 2026 synchronized-output block the
/// terminal freezes the screen until the mode is cleared; every other teardown step is
/// useless while that latch is held (C08 §7.3).
pub const RESTORE_SEQ: &[u8] = concat!(
    "\x1b[?2026l",         // end synchronized output FIRST
...
unsafe fn restore_raw() {
    ...
    if let Some(saved) = (*saved_ptr).as_ref() {
        // TCSAFLUSH, not TCSADRAIN: discard pending input as well as draining output. Any-
        // motion mouse tracking (mode 1003) can leave a large backlog of unread SGR events
        // in the tty input queue; RESTORE_SEQ above has already told the terminal to stop
        // sending them, but without this flush the queued bytes spill onto the shell prompt
        // as `^[[<..M` noise the instant we hand the terminal back.
        libc::tcsetattr(fd, libc::TCSAFLUSH, saved as *const libc::termios);
    }
}
    ...
    pub fn acquire(fd: RawFd) -> io::Result<Self> {
        if unsafe { libc::isatty(fd) } != 1 {
            return Err(io::Error::other(
                "not a tty (Terminal-Fenster needs an interactive terminal; \
                 use `terminal-fenster shot` for non-interactive capture)",
            ));
        }
...
mod tests {
    ...
    fn restore_sequence_undoes_every_mode_we_enable() {
        ...
        // Synchronized-output clear must precede everything else (C08 §7.3).
        assert!(
            s.find("\x1b[?2026l").unwrap() < s.find("\x1b[<u").unwrap(),
            "2026l must be first in RESTORE_SEQ"
        );
        // Stale images over the user's shell prompt are the most visible failure.
        assert!(
            s.contains("\x1b_Ga=d,d=A\x1b\\"),
            "must delete all images on restore"
        );
    }
    ...
    fn resize_flag_is_edge_triggered() {
        ...
        assert!(
            !take_resize(),
            "flag must clear so we do not resize in a loop"
        );
    }
