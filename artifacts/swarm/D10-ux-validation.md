...
**Machine:** macOS 26.1, Apple M4. Binary under test: `target/release/terminal-fenster` (terminal-fenster 0.1.0, built 31 Jul 21:49).
...
The single highest-value change is not a string at all — it is to give **`terminal-fenster doctor` a verdict line** (§4-A). Today doctor prints ~30 capability rows and stops; it never answers the one question a user runs it to answer: *"will `terminal-fenster open` work here, and if not, what do I do?"* A user on Apple Terminal sees `kitty graphics no`, `page viewport UNKNOWN`, and correctly-formatted rows, then runs `open` and hits a dead-end error (`could not determine terminal pixel size.`) that also tells them nothing. One computed verdict line closes both gaps at once. After that, fix the **seven error strings that state a failure but name no next action** (§3): the worst offender, `terminal-fenster: could not determine terminal pixel size.` (`main.rs:248`), is a reachable failure on the exact terminal in our test matrix (Apple Terminal, no `CSI 14t` reply) and gives the user zero direction. The project already ships two good exemplars to copy — the no-URL message (`main.rs:222`) gives an example command, and the missing-electron message (`main.rs:367`) names the env var to set — so the rewrites in §3 just bring the other messages up to that bar.

---

## 1. Evidence base — strings captured from the shipped binary

Run this session against `target/release/terminal-fenster`. These are the exact bytes a user sees; the audit in §3 quotes them verbatim.

```
$ terminal-fenster version
terminal-fenster 0.1.0                                            # exit 0

$ terminal-fenster frobnicate
terminal-fenster: unknown command "frobnicate"                    # exit 2, then prints full help

$ terminal-fenster open
terminal-fenster open: a URL is required
  e.g. terminal-fenster open example.com                          # exit 2

$ terminal-fenster open example.com          # stdin is not a tty (agent shell)
terminal-fenster: stdin is not a tty. Interactive browsing needs a terminal.   # exit 1

$ terminal-fenster doctor                    # stdin is not a tty
terminal-fenster doctor 0.1.0
  ...
  engine: $REPO/apps/engine/node_modules/.bin/electron   # exit 1
```

The help text (`terminal-fenster help`, exit 0) reproduces the block at `main.rs:76-92` verbatim, including the `TERMINAL_FENSTER_BACKEND  force a backend: kitty | unicode` line audited in §3.
...
|----|----------|---------|-------------------------------|------|
| **T-A1** | Help & version are discoverable and correct | `terminal-fenster help`; `terminal-fenster version`; `terminal-fenster bogus` | help exit 0 and contains `USAGE`, `open <url>`, `doctor`; version prints `terminal-fenster 0.1.0` exit 0; unknown command exits **2** and prints full help | any wrong exit code, or help missing the `open` subcommand |
| **T-A2** | Bad invocation self-corrects | `terminal-fenster open`; `printf '' \| terminal-fenster open example.com` | no-URL: exit **2**, stderr contains the `e.g. terminal-fenster open example.com` line; piped-stdin: exit **1**, stderr states stdin-is-not-a-terminal **and** how to run it right | exit 0 on either; or a message that names the problem but not the fix |
| **T-A3** | Missing engine routes to a fix | `TERMINAL_FENSTER_ENGINE=/nonexistent terminal-fenster open example.com` under a pty (`script`/`unbuffer`) | exit **1**; stderr names `TERMINAL_FENSTER_ENGINE` or "reinstall" as the fix | error names no next action |
| **T-A4** | Happy-path end-to-end (operator-run; Electron + graphics terminal, sandbox disabled) | `TERMINAL_FENSTER_EXIT_AFTER_MS=3000 TERMINAL_FENSTER_LOG=/tmp/bg.log terminal-fenster open https://example.com` in Ghostty | log contains `start`, `first-frame after <2500ms`, `bounded-run complete frames>0`; exit 0 | no first-frame line, frames = 0, or non-zero exit |
...
|----|---------|---------------------------|---------------|-------------------|
| **T-H1** | S1+S2 | "Open `example.com`." Bare shell, no docs, `terminal-fenster` on PATH. | ≥ 4/5 reach a rendered page within **3 min** using only `terminal-fenster help` / intuition; time-to-first-page recorded. | discoverability of the `open` subcommand; recovery from the plausible first guess `terminal-fenster example.com` (→ unknown-command + help) |
| **T-H2** | S2 (Apple Terminal) | "Find out whether Terminal-Fenster will work in this terminal, and why." | ≥ 4/5, reading `terminal-fenster doctor` **alone**, correctly conclude it will not render legibly here, name a reason (no kitty graphics / no pixel size) **and** a fix (use Ghostty/kitty/WezTerm). | doctor as a *diagnosis*. **Expected FAIL against current output** (no verdict line) → motivates §4-A. |
...
Per participant: task success matrix, time-to-first-page (T-H1), per-message comprehension (T-H3 before/after), severity-tagged issue list. Roll-up: task PASS/MARGINAL/FAIL, ship-gate status. Because screenshots are unavailable on a locked machine, capture evidence as `TERMINAL_FENSTER_LOG` transcripts + shell scrollback (both CI-friendly and reproducible), not images.
...
|---|-------------|------------------------------|---------|-----|
| 1 | `58` | `terminal-fenster 0.1.0` | PASS | version output, expected |
| 2 | `66` | `terminal-fenster: unknown command "x"` → then prints help | PASS | the auto-printed help *is* the next step |
| 3 | `76-89` | help USAGE / KEYS blocks | PASS | clear, example-shaped |
| 4 | `90` | `TERMINAL_FENSTER_BACKEND  force a backend: kitty \| unicode` | **PARTIAL** | `resolve_backend` (`207-214`) also accepts `sixel`; `iterm2` is auto-only. Help lists neither → a user trying to force sixel can't learn the value here |
| 5 | `89` | (help ENVIRONMENT) — `TERMINAL_FENSTER_LOG` absent | **PARTIAL** | the primary debugging lever is undocumented, so "how do I capture what went wrong" has no answer in help |
| 6 | `101-103` | doctor NOT-A-TTY status + explanation | PASS | tells the user to run it from a real terminal |
| 7 | `107,179` | `engine: NOT FOUND (set TERMINAL_FENSTER_ENGINE)` | PASS | names the env var to set — **exemplar** |
| 8 | `114` | `terminal-fenster: cannot acquire terminal: {e}` | **FAIL** | states failure, no next step |
...
| 15 | `222` | `a URL is required` / `e.g. terminal-fenster open example.com` | PASS | gives a runnable example — **exemplar** |
| 16 | `229` | `terminal-fenster: stdin is not a tty. Interactive browsing needs a terminal.` | **PARTIAL** | names the problem; weak on the cause (pipe/redirect/CI) and the fix |
| 17 | `236` | `terminal-fenster: {e}` (raw TtyGuard error) | **FAIL** | bare error, no prefix context, no action; inconsistent with #8 |
| 18 | `248` | `terminal-fenster: could not determine terminal pixel size.` | **FAIL (Critical)** | reachable on Apple Terminal (no `CSI 14t` reply); user gets zero direction |
| 19 | `265` | `terminal-fenster: cannot enable input protocols: {e}` | **FAIL** | no next step |
| 20 | `280` | `terminal-fenster: cannot start engine: {e}` | **PARTIAL** | quality rides on inner `{e}`; wrapper offers no log/diagnostic route |
| 21 | `367` | `electron not found; set TERMINAL_FENSTER_ENGINE to the engine directory` | PASS | names the env var — **exemplar** |
...
|---|--------|--------|------------------|
| 8 | `114` | `terminal-fenster: cannot acquire terminal: {e}` | `terminal-fenster: cannot put the terminal in raw mode: {e}`<br>`  This usually means stdin is redirected or the window closed.`<br>`  Run 'terminal-fenster doctor' in a normal terminal window to check.` |
...
| 16 | `229` | `terminal-fenster: stdin is not a tty. Interactive browsing needs a terminal.` | `terminal-fenster: stdin is not a terminal, so there is nothing to browse from.`<br>`  Run 'terminal-fenster open <url>' directly in a terminal window --`<br>`  not through a pipe, 'ssh host cmd', or a CI job.` |
| 17 | `236` | `terminal-fenster: {e}` | `terminal-fenster: cannot put the terminal in raw mode: {e}`<br>`  Run 'terminal-fenster open <url>' directly in a terminal, not through a pipe or redirect.` |
| 18 | `248` | `terminal-fenster: could not determine terminal pixel size.` | `terminal-fenster: this terminal does not report its pixel size, which Terminal-Fenster needs to size the page.`<br>`  Run 'terminal-fenster doctor' to see what it reports.`<br>`  Terminals that report it: Ghostty, kitty, or WezTerm.` |
| 19 | `265` | `terminal-fenster: cannot enable input protocols: {e}` | `terminal-fenster: could not switch the terminal into keyboard/mouse reporting: {e}`<br>`  Try another terminal, and include 'terminal-fenster doctor' output if you report this.` |
| 20 | `280` | `terminal-fenster: cannot start engine: {e}` | `terminal-fenster: could not start the browser engine: {e}`<br>`  Re-run with TERMINAL_FENSTER_LOG=/tmp/bg.log set to capture engine startup details.` |
| 22 | `377` | `cannot derive engine root` | `internal error: could not locate the engine root from {electron_path}. Please file a bug.` |
| 23 | `383` | `engine entrypoint missing: {path}` | `browser engine files are incomplete (missing {path}).`<br>`  Reinstall Terminal-Fenster, or point TERMINAL_FENSTER_ENGINE at a complete engine build.` |
| 24 | `423` | `engine did not connect within 30s` | `the browser engine did not start within 30s (it may have crashed on launch).`<br>`  Re-run with TERMINAL_FENSTER_LOG=/tmp/bg.log set, then check that file.` |
| 25 | `522` | `engine exited` | `the browser engine exited unexpectedly.`<br>`  Re-run with TERMINAL_FENSTER_LOG=/tmp/bg.log set to capture why.`  *(lines joined with `\r\n`)* |
| 26 | `528` | `engine read error: {e}` | `lost the connection to the browser engine: {e}.`<br>`  Re-run with TERMINAL_FENSTER_LOG=/tmp/bg.log set to capture why.`  *(lines joined with `\r\n`)* |
| 4 | `90` | `TERMINAL_FENSTER_BACKEND  force a backend: kitty \| unicode` | `TERMINAL_FENSTER_BACKEND  force a backend: kitty \| sixel \| unicode (iterm2 is auto-detected only)` |
| 5 | `89` | *(no TERMINAL_FENSTER_LOG line)* | add: `TERMINAL_FENSTER_LOG      write a diagnostic log to this file path` |

Each rewrite is verifiable by the T-H3 battery (§2.4): show it, ask "what next?", require ≥ 4/5 correct within 30 s. All twelve are worded so a correct next action is stated outright (run doctor / set TERMINAL_FENSTER_LOG / use Ghostty / reinstall / don't pipe).
...
- `locate_engine()` is `None` → `NO -- engine missing. Set TERMINAL_FENSTER_ENGINE or reinstall.`
...
Apply rows #4 and #5 from §3.2 (document `sixel`/`iterm2` backend semantics and the `TERMINAL_FENSTER_LOG` var). Low effort; closes the "how do I capture what went wrong" gap that every log-pointer rewrite in §3.2 depends on.
...
- **No screenshots** by design here — evidence is `TERMINAL_FENSTER_LOG` transcripts and shell scrollback, which are what a CI run would keep anyway.
