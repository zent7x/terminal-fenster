# B02 — Electron OSR Capability Probe (executed)

**Status:** EXECUTED 2026-08-01 — Electron 43.2.0 / Chromium 150, macOS 26.1 (Apple M4).
The probe from §6 is placed at `apps/engine/spike/b02-capability-probe.js` and was run in
`--mode=bitmap` (full, 21 stages) and `--mode=shared` (cross-check). Raw results:
`apps/engine/spike/out/b02-{bitmap,shared}.json` and `.log`.

**Verdict — Outcome A (§8): PARTIAL DAMAGE CONFIRMED in the bitmap/shipping path.** The
40×40 stimulus box at (600,400) produced dirty rects at exactly `(600,400,40,40)`,
`ratio 0.00123`, on 299/359 paints; the full-canvas control read `1.0` on every paint; idle
stopped painting. Damage tracking was the load-bearing unknown for the SSH story and it is
now settled affirmatively. Headline consequences (full detail folded into
`docs/adr/ADR-0001-browser-engine.md` "Consequences"):

- dirtyRect is in **device pixels**, same space as the bitmap → crop as-is, no CSS scaling
  (`dsf=2` → `(1200,800,80,80)`); rects are true boxes (`fullWidthPartialHeight 0`).
- fps throttling does **not** inflate damage (60/30/10 fps all `0.00123`) → H5 refuted.
- shared-texture `captureUpdateRect` independently reports the same partial damage → no
  native addon or Electron patch needed.
- Not consumed yet: `main.js` still sends the full bitmap; C08 owns the crop. This spike
  unblocks it.
- Caret blink unverified (headless session gave no OS focus, `hasFocus:false`, 1 paint);
  reconfirm interactively in a focused Ghostty window. Text-tick (78 on-box partial paints)
...
number is now measured, the result above supersedes the "expectation" framing.
