...
I wrote no file other than this one. **Disclosure:** I ran `touch crates/tf-term/src/tty.rs` to force a rebuild so I could capture the exact compiler warning text. That changed the file's mtime and nothing else; the bytes are untouched. It was still a core file and I should have found another way.
...
|---|---|---|
| `Cargo.toml` | Workspace root. Members: `crates/tf-term`, `crates/tf-proto`, `apps/cli`. `rust-version = "1.80"`, `license = "MIT OR Apache-2.0"`, `repository = ".../zent7x/terminal-fenster"` | `Cargo.toml:3,8-10` |
| `apps/cli/` | The `terminal-fenster` binary. 1039 lines, `src/main.rs` | `apps/cli/Cargo.toml`, `wc -l` |
| `crates/tf-term/` | tty guard, capability detection, kitty encoder, input decode, unicode fallback | 5 modules, 1897 lines |
| `crates/tf-proto/` | wire protocol | 284 lines |
| `apps/engine/` | Electron host. `package.json` name `@terminal-fenster/engine`, `private: true`, one dependency: `electron ^43.2.0` | `apps/engine/package.json` |
...
| `packages/mcp/` | MCP server, zero runtime deps, `bin: terminal-fenster-mcp`, `engines.node >= 22`, `"license": "UNLICENSED"` | `packages/mcp/package.json` |
...
### F09-1 (Critical, install/first-run) — `npm install` does not install a browser, and the first `terminal-fenster open` pays a hidden 14-second-to-forever bill with the output discarded
...
$ TERMINAL_FENSTER_ENGINE=<that dir> terminal-fenster doctor
...
`locate_engine()` tries three strategies in this order: `$TERMINAL_FENSTER_ENGINE`, the **compile-time** `CARGO_MANIFEST_DIR` dev layout (`main.rs:327-334`), then an exe-relative walk (`main.rs:336-348`). The second outranks the third, and it is baked into the binary at build time.

I proved the consequence in the exact form a user will hit it:

```
$ cargo install --path apps/cli --root <scratch>
$ <scratch>/bin/terminal-fenster doctor
...
```
# <T>/bin/terminal-fenster with a real <T>/engine/node_modules/.bin/electron present
$ <T>/bin/terminal-fenster doctor
  engine: $REPO/apps/engine/node_modules/.bin/electron
```
...
Side effect, verified with `strings`: the release binary embeds `$REPO/apps/cli`, so every distributed build leaks the builder's home directory path.
...
```
# <T>/real/bin/terminal-fenster with engine at <T>/real/engine, symlinked from <T>/linkdir/terminal-fenster
$ <T>/linkdir/terminal-fenster doctor
  engine: NOT FOUND (set TERMINAL_FENSTER_ENGINE)
```

This independently reproduces **B10 §6.5**, which found the same thing from the Homebrew direction and works around it with `write_env_script` setting `TERMINAL_FENSTER_ENGINE`. Two agents, two methods, same result. The install document must therefore tell users to **copy** the binary or use a wrapper, never `ln -s`.

Install layouts I verified as working (binary copied, not linked):

| Layout | Result |
|---|---|
| `<root>/bin/terminal-fenster` + `<root>/engine/` | FOUND |
| `<root>/terminal-fenster` + `<root>/engine/` (flat) | FOUND |
| `<root>/libexec/bg/bin/terminal-fenster` + `<root>/libexec/bg/engine/` | FOUND |
| symlink from anywhere | NOT FOUND |
| no engine present | `engine: NOT FOUND (set TERMINAL_FENSTER_ENGINE)`, exit 1 |

### F09-5 (Medium) — a wrong `TERMINAL_FENSTER_ENGINE` is silently ignored

```
$ TERMINAL_FENSTER_ENGINE=/nope/nothing terminal-fenster doctor
  engine: $REPO/apps/engine/node_modules/.bin/electron
```

The user set the variable, the variable was wrong, and Terminal-Fenster quietly used something else. `main.rs:320-325` only takes the env path if it exists; otherwise it falls through with no diagnostic. If the variable is set at all, a miss should be a loud error naming the path that was tried.
...
The MCP path already does the right thing (`packages/mcp/lib/engine.js:113-117` passes `--user-data-dir=<tmp>/profile`), but only when CDP is enabled. Set `TERMINAL_FENSTER_MCP_CDP=0` and the isolation silently disappears, because the `args.push` sits inside the `if (this.useCdp)` block.
...
| `$TMPDIR/terminal-fenster-mcp-*/` | 9 dirs, 356 KB | leaked MCP session dirs, each containing a Chromium profile |
| `$TMPDIR/terminal-fenster-mcp-audit.jsonl` | 9,661 bytes | persistent audit log of every URL navigated and element clicked. Nothing ever deletes it |

Two clean results worth recording: `~/Library/LaunchAgents` contains nothing of ours, the keychain contains nothing under `terminal-fenster`, and there were **zero** leftover `terminal-fenster-<pid>-<nanos>` socket directories, which confirms the CLI's own cleanup at `main.rs:676-677` works on the normal shutdown path. The MCP server's `rmSync` (`lib/engine.js:321`) does not survive a hard kill; that is where the nine leftovers came from.
...
| the `terminal-fenster` binary itself (release) | 619,424 bytes |
| **Steady state** | **~530 MB** |
...
### Installing Terminal-Fenster

Terminal-Fenster is a real Chromium browser that renders as pixels inside your terminal. It is two pieces that ship together: a Rust binary that owns your terminal, and an Electron host that runs Chromium offscreen and streams frames back. You need both.
...
**Your terminal matters more than anything else here.** Terminal-Fenster needs the kitty graphics protocol to draw real pixels. Without it you get a Unicode half-block fallback where layout and colour are visible but body text is not legible.
...
```sh
git clone https://github.com/zent7x/terminal-fenster    # NOT YET TRUE: no commits pushed
cd terminal-fenster
```
...
Produces `target/release/terminal-fenster` (about 620 KB). Expect one compiler warning from `crates/tf-term/src/tty.rs:133`; it is known and harmless.

Verify:

```sh
$ ./target/release/terminal-fenster version
terminal-fenster 0.1.0
```
...
`doctor` exits **1** when stdin is not a TTY, by design, because capability detection works by asking the terminal questions. Do not use it as a health check in CI; use `terminal-fenster version`, which exits 0.
...
```
~/.local/terminal-fenster/
  bin/terminal-fenster          <- copy of target/release/terminal-fenster
  engine/                 <- copy of apps/engine (package.json, src/, node_modules/)
```

```sh
mkdir -p ~/.local/terminal-fenster/bin
cp target/release/terminal-fenster ~/.local/terminal-fenster/bin/
cp -R apps/engine ~/.local/terminal-fenster/engine
```

**Do not symlink the binary onto your PATH.** `ln -s ~/.local/terminal-fenster/bin/terminal-fenster /usr/local/bin/terminal-fenster` produces a `terminal-fenster` that cannot find its own engine, because the discovery walk starts from the symlink's location rather than the real one. Use a wrapper instead:

```sh
printf '#!/bin/sh\nexec "$HOME/.local/terminal-fenster/bin/terminal-fenster" "$@"\n' > /usr/local/bin/terminal-fenster
chmod +x /usr/local/bin/terminal-fenster
```

Or set `TERMINAL_FENSTER_ENGINE` and skip the layout rules entirely:

```sh
export TERMINAL_FENSTER_ENGINE="$HOME/.local/terminal-fenster/engine"
```

Note that if `TERMINAL_FENSTER_ENGINE` points somewhere wrong, Terminal-Fenster currently ignores it silently and looks elsewhere rather than telling you. Confirm with `terminal-fenster doctor` that the path it reports is the one you meant.

`cargo install --path apps/cli` also works, but be aware of a rough edge: the resulting binary has your source-tree path compiled into it and will prefer that engine over any engine you install alongside it. Rebuild both together, and if you move or delete the source tree, set `TERMINAL_FENSTER_ENGINE`.

#### Environment variables

| Variable | Effect |
|---|---|
| `TERMINAL_FENSTER_ENGINE` | directory containing `node_modules/.bin/electron` and `src/main.js` |
| `TERMINAL_FENSTER_BACKEND` | force `kitty`, `sixel`, or `unicode` |
| `TERMINAL_FENSTER_LOG` | append diagnostics to this file. Never logs to stdout, which is the graphics channel |
| `TERMINAL_FENSTER_MCP_*` | MCP server: `_LOG`, `_AUDIT`, `_PROFILE`, `_CDP=0`, `_WIDTH`, `_HEIGHT` |
...
$TMPDIR/terminal-fenster-<pid>-<nanos>/                        control socket, 0700, deleted on clean exit
$TMPDIR/terminal-fenster-mcp-XXXXXX/                           MCP session dir, includes a Chromium profile
$TMPDIR/terminal-fenster-mcp-audit.jsonl                       MCP audit log of URLs and clicks. Never
                                                        deleted automatically
```

Terminal-Fenster installs no launch agents, no login items, nothing under `/Library`, and no keychain entries. Verified by sweep.

> **Known issue — your browsing data is not in a Terminal-Fenster-specific directory.** The engine does not set an application name, so Chromium writes cookies, localStorage, and cache to Electron's generic default at `~/Library/Application Support/Electron`. Every other unpackaged Electron app on your machine uses that same directory. Two consequences: your Terminal-Fenster browsing state sits alongside other apps' data, and a clean uninstall cannot simply delete that folder. See the uninstall section for how to handle it. This is tracked as a pre-1.0 blocker.
...
Everything Terminal-Fenster creates, in order, with the shared-resource cases called out so you can decide.

**1. The project and its build output.**

```sh
rm -rf /path/to/terminal-fenster
```
...
```sh
rm -rf ~/.local/terminal-fenster
rm -f ~/.cargo/bin/terminal-fenster
rm -f /usr/local/bin/terminal-fenster          # the wrapper, if you made one
```
...
That glob covers the CLI's socket directories, the MCP session directories, and `terminal-fenster-mcp-audit.jsonl`. If you pointed `TERMINAL_FENSTER_MCP_AUDIT`, `TERMINAL_FENSTER_MCP_LOG`, or `TERMINAL_FENSTER_LOG` elsewhere, delete those files too. **The audit log records every URL you navigated to and every element you clicked**, so if privacy is the reason you are uninstalling, do not skip it.
...
If you do not develop with Electron and have never run another unpackaged Electron app, this directory is Terminal-Fenster's and you can remove it:
...
**7. Leave these alone.** `~/.cargo/registry` and `~/.cargo/git` are your Rust toolchain's shared caches and are not Terminal-Fenster's to remove. `~/Library/Preferences/com.github.Electron.plist` may predate your Terminal-Fenster install; on the reference machine it did, by three months.

**Verify:**

```sh
ls -d ~/Library/Application\ Support/Electron 2>/dev/null
ls -d "${TMPDIR:-/tmp}"/terminal-fenster-* 2>/dev/null
command -v terminal-fenster
```
...
| `cannot start engine: electron not found` | no engine in any search location | set `TERMINAL_FENSTER_ENGINE` |
| `doctor` says `engine: NOT FOUND` but you installed it | binary is a symlink, or engine is not in a sibling `engine/` dir | copy the binary instead of linking, or set `TERMINAL_FENSTER_ENGINE` |
| `doctor` shows an engine path you did not install | build-tree path compiled into the binary wins over your install | set `TERMINAL_FENSTER_ENGINE` explicitly |
...
| macOS asks whether "Electron" may accept incoming connections | MCP/CDP mode binds a debug port from an ad-hoc-signed binary | deny it; CDP is local-only. Or `TERMINAL_FENSTER_MCP_CDP=0` |
| `doctor` exits 1 in CI | non-TTY, by design | use `terminal-fenster version` |
...
| V2 | Test suite | `cargo test --workspace` | 0 failed | RAN — **96 passed, 0 failed** (tf-proto 12, tf-term 70, terminal-fenster 14). The brief's figure of 87 is stale |
| V3 | Zero warnings | `cargo build --release 2>&1 \| grep -c warning:` | 0 | RAN — **FAIL**, 1 warning at `crates/tf-term/src/tty.rs:133` |
| V4 | Binary runs without a TTY | `terminal-fenster version` | prints version, exit 0 | RAN — PASS |
...
| V10 | `doctor` resolves the engine | `terminal-fenster doctor` | engine line, not `NOT FOUND` | RAN — PASS |
...
| V16 | Bad `TERMINAL_FENSTER_ENGINE` errors loudly | `TERMINAL_FENSTER_ENGINE=/nope doctor` | non-zero, names the path | RAN — **FAIL**, silently falls back (F09-5) |
...
| # | Check | Command | Pass criterion | Status |
|---|---|---|---|---|
| V18 | End-to-end first frame | `TERMINAL_FENSTER_EXIT_AFTER_MS=4000 TERMINAL_FENSTER_LOG=/tmp/bg.log terminal-fenster open https://example.com` | log shows `first-frame`, terminal restored | BLOCKED under the agent sandbox (`bootstrap_look_up … Permission denied`). Previously verified in Ghostty 1.3.1: ready 212 ms, first frame 366 ms |
| V19 | Socket directory cleanup | `ls "$TMPDIR"/terminal-fenster-*` after a session | none | RAN — PASS, zero CLI leftovers |
| V20 | MCP temp cleanup | same, `terminal-fenster-mcp-*` | none | RAN — **FAIL**, 9 leftovers (356 KB) |
| V21 | Uninstall leaves nothing | run §4's uninstall, then sweep `~/Library` for files newer than install time | no Terminal-Fenster-attributable residue | BLOCKED as a full test (would delete another agent's live data). The **inventory** it must cover is measured in F09-7 |
...
app.setPath('userData', process.env.TERMINAL_FENSTER_PROFILE ||
            path.join(app.getPath('appData'), 'Terminal-Fenster', 'engine-profile'));
```
