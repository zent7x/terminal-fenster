# B01 — BlackGlass Architecture RFC
...
**The core source changed while this RFC was being written.** Between my first read and my last verification pass, `apps/cli/src/main.rs` grew 932 → 1,039 lines, `crates/bg-term/src/caps.rs` grew 319 → 373, and `apps/engine/src/main.js` grew 290 → 309. All line citations below are therefore anchored to an explicit snapshot; if the sizes no longer match, re-verify before acting on a citation.

| File | Lines | mtime |
|---|---|---|
| `apps/cli/src/main.rs` | 1,039 | 31 Jul 21:49:04 |
| `apps/engine/src/main.js` | 309 | 31 Jul 21:46:55 |
| `crates/bg-term/src/caps.rs` | 373 | 31 Jul 21:44:13 |
| `crates/bg-proto/src/lib.rs` | 284 | 31 Jul 21:23:03 |
| `crates/bg-term/src/unicode.rs` | 164 | 31 Jul 21:20:15 |
| `crates/bg-term/src/input.rs` | 768 | 31 Jul 21:18:45 |
| `crates/bg-term/src/kitty.rs` | 396 | 31 Jul 21:17:06 |
| `crates/bg-term/src/tty.rs` | 256 | 31 Jul 21:15:51 |
| `crates/bg-term/src/b64.rs` | 89 | 31 Jul 21:14:56 |
| `crates/bg-term/src/lib.rs` | 124 | 31 Jul 21:14:42 |

| Fact | Value | How obtained (this session) |
|---|---|---|
| Workspace tests | **96 passing, 0 failing** — bg-proto 12, bg-term 70, blackglass 14 | `cargo test --workspace`, final pass |
| Test count drift during this RFC | 91 → 96 | two runs, ~40 minutes apart |
| Rust source size | 3,802 lines across 9 files | `wc -l` |
| Workspace members | `crates/bg-term`, `crates/bg-proto`, `apps/cli` | `Cargo.toml:3` |
| Crate coupling | bg-term and bg-proto **do not reference each other** | `crates/*/Cargo.toml` — neither declares the other |
| bg-proto dependencies | **none at all** | `crates/bg-proto/Cargo.toml` has no `[dependencies]` |
...
flowchart TB
    ...
    subgraph CORE["blackglass — Rust, single process, single thread"]
        direction TB
        TTYG["tty::TtyGuard<br/>raw mode, RAII, signal + panic restore<br/>bg-term/src/tty.rs:82"]
        CAPS["caps::detect<br/>6 query/response probes, 300 ms each<br/>bg-term/src/caps.rs:128"]
...
Process ownership is strictly hierarchical: the shell owns `blackglass`; `blackglass` owns the Electron main process (`Command::spawn`, `apps/cli/src/main.rs:404-411`); Electron owns the Chromium tree. Both directions of orphan prevention are wired — the core kills the child on teardown after a 1500 ms grace (`apps/cli/src/main.rs:657-676`), and the engine exits when the socket closes (`apps/engine/src/main.js:307`).
...
sequenceDiagram
    ...
    participant C as blackglass core
...
1. `crates/bg-term/src/tty.rs:169-171` — `enable_input_protocols` writes mode-setting sequences to `io::stdout()`.
2. `crates/bg-term/src/caps.rs:116-120` — `query()` writes each probe sequence to `io::stdout()`; `detect` also writes an image-cleanup sequence directly at `:150-153`.
3. `apps/cli/src/main.rs:899-901` — `Renderer::present` writes frames and the status bar to `io::stdout()`.

Plus a fourth, deliberately outside Rust's stdio: `crates/bg-term/src/tty.rs:56-60` uses raw `libc::write` in the signal path, which is correct and must stay that way — `println!` is not async-signal-safe.
...
The core trusts the engine's length prefix without bound. `MessageReader::next_message` (`crates/bg-proto/src/lib.rs:81-93`) reads a `u32` length and waits for that many bytes, so a corrupt or hostile prefix asks the core to buffer up to 4 GiB. The frame payload is separately validated — a frame with fewer pixels than `w*h*4` is dropped rather than rendered (`apps/cli/src/main.rs:834-836`, with a regression test) — but that check happens *after* the allocation.

Today the engine is our own process on the far side of a `0600` socket, so this is a robustness issue rather than a live vulnerability. It becomes a real trust boundary the moment A07's topology (b′) lands and the far side of that socket is a remote host over `ssh -T`. **A length cap belongs in `bg-proto` before, not after, remote transport ships.**
...
| Parser | Hand-rolled field extractors, `json_get_str` / `json_get_bool` (`crates/bg-proto/src/lib.rs:125,158`) | `FrameHeader::parse`, fixed offsets (`crates/bg-proto/src/lib.rs:33`) |
...
The deliberate asymmetry is documented at `crates/bg-proto/src/lib.rs:7-9` and it is the right call: JSON keeps low-rate messages readable in logs while a 5–8 MB BGRA buffer through JSON would be indefensible.
...
```
blackglass/
├─ crates/bg-term/     library — everything that touches the user's terminal
...
**I1 — bg-proto has zero dependencies.** Verified: `crates/bg-proto/Cargo.toml` declares no `[dependencies]` section at all. This is what makes the protocol trivially fuzzable and portable to a future remote-renderer binary. It also motivated the hand-rolled JSON extractors, which the source justifies at `crates/bg-proto/src/lib.rs:120-124`: both ends of the protocol are ours, the messages are flat, and a full JSON parser is a dependency and an attack surface that buys nothing here. That reasoning holds *only while both ends are ours* — it must be revisited for topology (b′).

**I2 — bg-term and bg-proto never reference each other.** Verified from both manifests. Terminal knowledge and engine knowledge meet in exactly one place, `apps/cli/src/main.rs`, and nowhere else. This is the property that lets the engine be swapped (ADR-0001's stated reversal path) without touching the terminal code.

**I3 — nothing below `apps/` knows what a browser is.** `bg-term` has no concept of a page, a URL, or a frame source; it converts pixels to escapes.
...
- cannot be depended on by a second binary, which A07's topology (b′) explicitly requires (`blackglass-render` local, engine remote);
- cannot be depended on by a future non-interactive `blackglass shot`, which `crates/bg-term/src/tty.rs:88-90` already advertises in an error message that no code implements;
- is testable only through `#[cfg(test)]` in the binary crate — 14 tests today for the component that holds the hot path, against 70 for `bg-term`.
...
`Capabilities::best_backend` can return `Backend::Iterm2` or `Backend::Sixel` (`crates/bg-term/src/caps.rs:51-61`), and `BLACKGLASS_BACKEND=sixel` is accepted (`apps/cli/src/main.rs:209-212`). But `Renderer::present` matches only `Backend::Kitty` and routes **everything else** to the Unicode half-block path (`apps/cli/src/main.rs:872`). So `blackglass doctor` can name a backend that `blackglass open` will not use, which is a diagnostic tool telling the user something false.

The practical blast radius shrank while this RFC was being written: `crates/bg-term/src/caps.rs:187-191` now records that iTerm2 3.6.9 was measured to support the kitty graphics protocol, which the detector prefers anyway, making the `Iterm2` arm largely moot. That is a good outcome for users and it does not fix the type-level problem — `best_backend` can still return a variant the renderer silently reinterprets. Either implement the arms or narrow the return type to what the renderer can honor.
...
|---|---|---|---|---|
| F1 | **Terminal resized mid-session** | none | Nothing. No `SIGWINCH` handler exists anywhere (grep: zero hits in `apps/cli/src` and `crates/bg-term/src`). Geometry is captured once in `cmd_open` and passed by value into `run()`. The engine's `resize` command (`apps/engine/src/main.js:231`) is never sent (grep: zero senders). | The page keeps rendering at the old size; image and status row drift out of registration and pointer mapping goes wrong. **This is the most likely first bug a real user hits.** Needs SIGWINCH → re-query `CSI 14t` → send `resize` → reposition status row. |
...
| F4 | **Engine fails to start / never connects** | 30 s accept deadline | Clean error, terminal restored (`apps/cli/src/main.rs:415`). | 30 s is a long time to stare at a dead terminal for what ADR-0001 measured as a ~1–2 s cold start and this session measured at 212 ms warm. And the child's stderr is `Stdio::null()` (`apps/cli/src/main.rs:410`), so **the reason is unrecoverable**. Redirect engine stderr into `$BLACKGLASS_LOG`. |
| F5 | **Process killed by signal** | signal handler | Restores tty, resets the disposition to default, re-raises so the exit status is a genuine 128+N (`crates/bg-term/src/tty.rs:67-75`). SIGINT/TERM/HUP/QUIT covered; SIGPIPE ignored so a write to a vanished terminal surfaces as EPIPE rather than killing us mid-teardown (`:137`). | The child is **not** reaped on this path — `Session::shutdown` never runs. The Electron tree survives until it notices the socket closed (`apps/engine/src/main.js:307`), which is the right backstop, but it is the only one. |
| F6 | **Panic** | panic hook | Restores tty *before* the default hook prints, so the backtrace is readable with OPOST off (`crates/bg-term/src/tty.rs:126-129`). `panic = "unwind"` is pinned in `Cargo.toml:20` with the reason in a comment — abort would skip the hook and corrupt the tty. | None. This is exemplary. |
...
| **ADR-0014** | Observability and clock discipline | One monotonic timeline (`CLOCK_UPTIME_RAW`) across Rust and Node, all diagnostics to `$BLACKGLASS_LOG` and never to stdout, and engine stderr captured rather than discarded. |
...
| **ADR-0019** | Test strategy and CI gates | Unit tests cover protocol and encoder invariants, `tests/` holds capability-replay and bounded-run end-to-end tests using `BLACKGLASS_EXIT_AFTER_MS`, and no gate is ever weakened to make a build pass. |
...
3. **UNVERIFIED — 1 KiB chunk anomaly.** A10 measured 1 KiB PTY writes as 3.3× faster than 8 KiB, hypothesised as macOS clist/`TTYHIWAT` behavior. `kitty::MAX_CHUNK` is 4096 because that is the protocol's base64 limit (`crates/bg-term/src/kitty.rs:25`), which is a *different* constant from the write-syscall chunk size. The core currently does one `write_all` of the whole buffer (`apps/cli/src/main.rs:900`) — neither chunking regime. Worth measuring before ADR-0014.
